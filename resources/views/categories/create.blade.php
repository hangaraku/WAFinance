    ...
    <div class="bg-white shadow-sm border-b">
        <div class="px-4 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-semibold text-gray-900">Tambah Kategori</h1>
                    <p class="text-sm text-gray-600">Buat kategori baru untuk transaksi dan anggaran</p>
                </div>
                    ...
                    @foreach($icons as $iconName => $iconPath)
                        <label class="relative">
                            <input type="radio" 
                                   name="icon" 
                                   value="{{ $iconName }}" 
                                   {{ old('icon', 'dots-horizontal') === $iconName ? 'checked' : '' }}
                                   class="sr-only peer">
                    ...
                    @foreach($colors as $colorValue => $colorName)
                        <label class="relative">
                            <input type="radio" 
                                   name="color" 
                                   value="{{ $colorValue }}" 
                                   {{ old('color', '#6B7280') === $colorValue ? 'checked' : '' }}
                                   class="sr-only peer">
...
document.querySelectorAll('input[name="type"]').forEach(radio => {
    radio.addEventListener('change', function() {
        ...
        if (this.value === 'expense') {
            ...
            // Auto-select income-related icon
            iconInputs.forEach(input => {
                if (input.value === 'currency-dollar') {
                    input.checked = true;
                }
            });
        }
