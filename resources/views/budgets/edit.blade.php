...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white shadow-sm border-b border-gray-200">
        <div class="px-4 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-semibold text-gray-900">Edit Budget</h1>
                    <p class="text-sm text-gray-600">Update anggaran untuk kategori pengeluaran</p>
                </div>
    ...
    <!-- Form -->
    <div class="px-4 py-6">
        <form action="{{ route('budgets.update', $budget) }}" method="POST" class="max-w-2xl mx-auto">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="month" required
                            ...
                            <option value="1" {{ old('month', $budget->month) == 1 ? 'selected' : '' }}>Januari</option>
                            <option value="2" {{ old('month', $budget->month) == 2 ? 'selected' : '' }}>Februari</option>
                            <option value="3" {{ old('month', $budget->month) == 3 ? 'selected' : '' }}>Maret</option>
                            <option value="4" {{ old('month', $budget->month) == 4 ? 'selected' : '' }}>April</option>
                            <option value="5" {{ old('month', $budget->month) == 5 ? 'selected' : '' }}>Mei</option>
                            <option value="6" {{ old('month', $budget->month) == 6 ? 'selected' : '' }}>Juni</option>
                            <option value="7" {{ old('month', $budget->month) == 7 ? 'selected' : '' }}>Juli</option>
                            <option value="8" {{ old('month', $budget->month) == 8 ? 'selected' : '' }}>Agustus</option>
                            <option value="9" {{ old('month', $budget->month) == 9 ? 'selected' : '' }}>September</option>
                            <option value="10" {{ old('month', $budget->month) == 10 ? 'selected' : '' }}>Oktober</option>
                            <option value="11" {{ old('month', $budget->month) == 11 ? 'selected' : '' }}>November</option>
                            <option value="12" {{ old('month', $budget->month) == 12 ? 'selected' : '' }}>Desember</option>
                        </select>
                        @error('month')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 mb-6">
                ...
                <textarea name="notes" rows="3" 
                          ...
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent">{{ old('notes', $budget->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
